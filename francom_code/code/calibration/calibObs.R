...
out.name<-'calibBassObs_noDiscrep'
...
#   print(it)
# }
# save(bassCat,file='~/Desktop/bassCatRaw.rda')

# load('~/Google Drive/MARS/applications/flex/calib/dat/bassCatRaw.rda')
...
nc<-4 # number of cores to use in prediction
...
for(i in 2:nmcmc){
  ...
  delta<-dmod.prior$yhat[itd,]#rnorm(n,dmod.m,dmod.s)
  gam[i]<-0#rtruncnorm(1,0,2,sum(delta*(y-emuSamp.curr$eta))/sum(delta^2),s2[i]/sum(delta^2))
...
#burn<-1:nburn
burn<-c(1:10000,seq(10001,40000,3)) # burn and thin
#X<-x
